import { Box, Button, Card, CardActions, CardContent, CardHeader, Chip, Fab, Icon, IconButton, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography } from '@mui/material';
...
export const BrandList: React.FC = () => {
  ...
  return (<>
    <Box sx={{maxWidth: 960, width: '100%', m: '0 auto'}}>
      {list.length ? list.map((brand) => (
        <Card sx={{w: '100%', mb: '16px'}} key={brand.id}>
          <CardHeader title={brand.name} titleTypographyProps={{variant: 'h6'}}>
          </CardHeader>
          <CardContent>
              ...
              <TableBody>
                {brand.flavors.map((flavor) => (
                  <TableRow key={flavor.id}>
                    ...
                    <TableCell>
                      <IconButton onClick={() => dispatch(brandsActions.editFlavor({brand: brand.id, flavor: flavor.id}))}>
                        <Icon>edit</Icon>
                      </IconButton>
                    </TableCell>
                  </TableRow>
          ...
          </TableContainer> : <Typography component="p" variant="caption" sx={{textAlign: 'center'}}>
            This brand doesn't have any flavors, yet
          </Typography>}
          </CardContent>
          <CardActions>
            <Button onClick={() => dispatch(brandsActions.newFlavor({brand: brand.id}))}>Add Flavor</Button>
            <Button onClick={() => dispatch(brandsActions.editBrand(brand.id))}>Edit Brand</Button>
          </CardActions>
        </Card>
