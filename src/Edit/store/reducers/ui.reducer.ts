...
export const uiReducer = createReducer<UIState>({drawerOpen: false,unsavedChanges: false}, (builder) => 
  builder.addCase(uiActions.toggleDrawer, (state, _action) => ({...state, drawerOpen: !state.drawerOpen}))
    ...
    .addMatcher(
      (action) => action.type === fileActions.loadFileSucceeded.type ||
      ...
      (action) => [
        brandsActions.saveBrand.type, brandsActions.saveFlavor.type,
        mealActions.saveMeal.type, mealActions.deleteMeal.type,
