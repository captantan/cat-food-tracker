...
export const loadContentSucceeded = createAction<ContentState>(actionPrefix + 'Load Succeeded');
export const loadContentFailed = createAction<number>(actionPrefix + 'Load Failed');

export function loadContent(path: string): ThunkAction<Promise<void>, State, unknown, Action> {
  return async function loadFileThunk(dispatch, _getState): Promise<void> {
    try {
      ...
      const fileResponse = await fetch('https://graph.microsoft.com/v1.0/me/drive/root' + (path ? `:/${encodeURIComponent(path)}:/` : '/') + 'children', {
