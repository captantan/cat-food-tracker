...
export const createStarted = createAction<string /* new file path */>(actionPrefix + 'Create Started');
export const createSucceeded = createAction(actionPrefix + 'Create Succeeded');
export const createFailed = createAction<number>(actionPrefix + 'Create Failed');

export function createFile(path: string, navigate: NavigateFunction): ThunkAction<Promise<void>, State, unknown, Action> {
  return async function loadFileThunk(dispatch, _getState): Promise<void> {
    try {
      ...
      const uploadResponse = await fetch('https://graph.microsoft.com/v1.0/me/drive/root:' + (path.startsWith('/') ? '' : '/') + path + ':/content', {
